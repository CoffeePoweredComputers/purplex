...
class DockerExecutionService:
    ...
    def __init__(self):
        ...
        self.docker_image = security_config.get(
        ...
        self.health_check_interval = security_config.get(
            "POOL_HEALTH_CHECK_INTERVAL", 60
        )  # Check every 60s
        self.container_max_age = security_config.get(
            "POOL_CONTAINER_MAX_AGE", 3600
        )  # Rotate after 1 hour
    ...
    def _ensure_image_exists(self):
        ...
        try:
            ...
            logger.warning(
                f"Docker image {self.docker_image} not found, attempting to build..."
            )
    ...
    def _build_sandbox_image(self):
        ...
        try:
            image, logs = self.docker_client.images.build(
                path=dockerfile_path, tag=self.docker_image, rm=True, forcerm=True
            )
    ...
    def _init_pool(self):
        ...
        logger.info(
            f"Container pool initialized with {len(self.container_pool)} containers"
        )
...
"""
        ...
        try:
            container = self.docker_client.containers.run(
                ...
                user="1000:1000",
                working_dir="/sandbox",
                # nosec B108: tmpfs paths are intentional for Docker sandbox isolation
                tmpfs={
            ...
            if container.status == "running":
                ...
                logger.warning(
                    f"Container {container.id[:12]} not running, status: {container.status}"
                )
    ...
    def _start_health_monitor(self):
        ...
        logger.info(
            f"Started container pool health monitor (interval: {self.health_check_interval}s)"
        )
            ...
            with self.pool_lock:
                ...
                for container in containers_to_check:
                    ...
                    try:
                        ...
                        is_restarted_too_much = (
                            restart_count > self.max_restart_attempts
                        )
                        ...
                        if not is_running:
                            ...
                            try:
                                ...
                                if new_container:
                                    ...
                                    logger.debug(
                                        f"Created replacement container {new_container.id[:12]}"
                                    )
                            except Exception as e:
                                logger.error(
                                    f"Failed to create replacement container: {e}"
                                )
                        ...
                        logger.warning(
                            f"Error checking container {container_id[:12]}: {e}"
                        )
    ...
    def get_pool_metrics(self):
        ...
        with self.pool_lock:
            ...
            hit_rate = (
                (self.pool_metrics["pool_hits"] / total_requests * 100)
                if total_requests > 0
                else 0.0
            )
            avg_wait_time = (
                (self.pool_metrics["pool_wait_time_total"] / total_requests)
                if total_requests > 0
                else 0.0
            )
            ...
            while self.container_pool:
                ...
                try:
                    ...
                    if container.status == "running":
                        ...
                        logger.debug(
                            f"Retrieved container {container.id[:12]} from pool (wait: {wait_time:.3f}s)"
                        )
                        ...
                        logger.warning(
                            f"Removing unhealthy container {container.id[:12]} from pool"
                        )
                        try:
                            container.remove(force=True)
                        except Exception:
...
"""
                ...
                if cleanup_result.exit_code != 0:
                    logger.warning(
                        f"Container {container.id[:12]} cleanup failed, removing from pool"
                    )
                    container.remove(force=True)
                    return
                ...
                logger.warning(
                    f"Container {container.id[:12]} cleanup error: {cleanup_error}, removing"
                )
                container.remove(force=True)
                return
            ...
            with self.pool_lock:
                if len(self.container_pool) < self.pool_size:
                    self.container_pool.append(container)
                    logger.debug(
                        f"Returned container {container.id[:12]} to pool (cleaned)"
                    )
                else:
            ...
            try:
                if hasattr(self, "docker_client") and self.docker_client:
                    containers = self.docker_client.containers.list(
                        all=True, filters={"label": "purplex-pool=true"}
                    )
                    if containers:
                        logger.info(
                            f"Cleaning up {len(containers)} orphaned pool containers"
                        )
                        for container in containers:
                            try:
                                container.remove(force=True)
                                logger.debug(
                                    f"Removed orphaned pool container {container.id[:12]}"
                                )
                            except Exception:
                                pass
            except Exception as e:
...
'''
    ...
    def _execute_in_pooled_container(self, code: str) -> dict:
        ...
        try:
            ...
            if not container:
                ...
                logger.warning(
                    "Failed to get pooled container, falling back to new container"
                )
                return self._execute_in_new_container(code)
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Execution exceeded {self.max_execution_time}s timeout"
                    )
                    try:
                        container.kill()
                        container.remove(force=True)
                ...
                if exec_result.output:
                    ...
                    if len(output_bytes) > MAX_OUTPUT_SIZE:
                        # Truncate output
                        output = output_bytes[:MAX_OUTPUT_SIZE].decode(
                            "utf-8", errors="ignore"
                        )
                        output += (
                        ...
                        logger.warning(
                            f"Output truncated: {len(output_bytes)} bytes > {MAX_OUTPUT_SIZE} bytes"
                        )
                    else:
    ...
    def _execute_in_new_container(self, code: str) -> dict:
        ...
        container_config = {
            ...
            "cpu_quota": self.max_cpu_percent
            * 1000,  # Convert percentage to microseconds
            ...
            "ulimits": (
                [docker.types.Ulimit(name="nofile", soft=64, hard=64)] if docker else []
            ),
        }
        ...
        try:
            ...
            if len(logs_bytes) > MAX_OUTPUT_SIZE:
                ...
                logs += f"\n\n... (output truncated at {MAX_OUTPUT_SIZE} bytes)"
                logger.warning(
                    f"Output truncated: {len(logs_bytes)} bytes > {MAX_OUTPUT_SIZE} bytes"
                )
            else:
            ...
            return {"success": False, "output": "", "error": "Docker execution error"}
        except Exception as e:
