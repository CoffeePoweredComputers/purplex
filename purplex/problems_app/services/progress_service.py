...
class ProgressService:
    ...
    def get_problem_progress(user_id: int, problem_slug: str) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                progress = (
                    ProgressRepository.get_by_ids(user_id, problem.id)
                    if problem
                    else None
                )
                return {
    ...
    def get_or_create_problem_set_progress(
        user: "User", problem_set: "ProblemSet", course: Optional["Course"] = None
    ) -> "UserProblemSetProgress":
        """
    ...
    def get_user_all_progress(
        user: "User", course: Optional["Course"] = None
    ) -> "models.QuerySet":
        """
        ...
        for progress in progresses:
            ...
            if course_id not in course_data:
                course_data[course_id] = {
                    "course_name": (
                        progress.course.name if progress.course else "No Course"
                    ),
                    "problem_sets": [],
                }
            ...
            summary = {
                "overall": {
                    ...
                    "completion_percentage": int(
                        (total_completed / total_problems * 100)
                        if total_problems > 0
                        else 0
                    ),
                },
    ...
    def get_progress_context(
        ...
        try:
            ...
            problem_set = None
            if problem_set_slug:
                problem_set = ProblemRepository.get_problem_set_by_slug(
                    problem_set_slug
                )
                if not problem_set:
            ...
            if problem_set and course:
                progress = (
                    ProgressRepository.filter_by_ids(
                        user_id=user_id, problem_id=problem.id
                    )
                    .filter(problem_set=problem_set, course=course)
                    .first()
                )
            elif problem_set:
                progress = (
        ...
        try:
            ...
            if course:
                ...
                if progress.total_problems == 0:
                    progress.total_problems = ProblemRepository.count_problems_in_set(
                        problem_set
                    )
                    progress.save()
            else:
            ...
            progress_qs = ProgressRepository.filter_by_ids(user_id=user.id).filter(
                problem_set=problem_set, course=course
            )
            ...
            for (
                membership
            ) in ProblemSetMembershipRepository.get_problem_set_memberships(
                problem_set
            ):
                ...
                if last_submission and hasattr(last_submission, "segmentation"):
                    segmentation_passed = (
                        last_submission.segmentation.passed
                        if last_submission.segmentation
                        else None
                    )
                ...
                problems_with_progress.append(
                    {
                        ...
                        "status": (
                            user_progress.status if user_progress else "not_started"
                        ),
    ...
    def get_last_submission_with_context(
        ...
        problem = ProblemRepository.get_problem_by_slug(problem_slug)
        if not problem:
            return {"problem": None, "submission": None, "has_submission": False}
    ...
    def process_submission(submission) -> "UserProgress":
        ...
        logger.info(
            f"[ProgressService] Processing submission {submission.submission_id}"
        )
        logger.info(
            f"[ProgressService] User: {submission.user.username}, Problem: {submission.problem.slug}"
        )
        logger.info(
            f"[ProgressService] Score: {submission.score}, Problem Set: {submission.problem_set.slug if submission.problem_set else 'None'}"
        )

        with transaction.atomic():
            # Get or create progress record with full context
            progress = ProgressService._get_or_create_progress_for_submission(
                submission
            )
                ...
                if not progress.completed_at:
                    ...
                    if progress.first_attempt:
                        progress.days_to_complete = (
                            progress.completed_at - progress.first_attempt
                        ).days
            ...
            if old_status != status:
                logger.info(
                    f"[ProgressService] Status changed: {old_status} -> {status}"
                )
            if old_score != progress.best_score:
                logger.info(
                    f"[ProgressService] Best score changed: {old_score} -> {progress.best_score}"
                )

            logger.info(
                f"[ProgressService] Saving progress - is_completed: {progress.is_completed}, status: {status}, grade: {grade}"
            )
            ...
            ProgressService._invalidate_progress_cache(
                ...
                problem_set_id=(
                    progress.problem_set.id if progress.problem_set else None
                ),
            )
            ...
            logger.info(
                f"[ProgressService] Completed processing submission {submission.submission_id}"
            )
    ...
    def _get_or_create_progress_for_submission(submission) -> "UserProgress":
        ...
        try:
            ...
            try:
                ...
                if lock_duration > 0.1:  # Log if lock acquisition took >100ms
                    logger.info(
                        f"[ProgressService] Lock acquired in {lock_duration:.3f}s"
                    )
            except DatabaseError as e:
                lock_duration = time.time() - lock_start
                logger.warning(
                    f"[ProgressService] Progress locked by another request after {lock_duration:.3f}s: {e}"
                )
            ...
            if progress:
                logger.info(
                    f"[ProgressService] Progress record FOUND with ID: {progress.id} (LOCKED)"
                )
                created = False
            else:
                ...
                try:
                    ...
                    logger.info(
                        f"[ProgressService] Progress record CREATED with ID: {progress.id}"
                    )
                    created = True
                    ...
                    logger.info(
                        f"[ProgressService] Progress record FOUND after race condition with ID: {progress.id} (LOCKED)"
                    )
                    created = False
        except DatabaseError as e:
            logger.error(
                f"[ProgressService] Database lock error after retries: {str(e)}"
            )
            raise ValueError(
        ...
        if created and submission.problem_set:
            try:
                ...
                logger.info(
                    "[ProgressService] Migrating old progress record without problem_set"
                )
                ...
                progress.save()
                old_progress.delete()
                logger.info("[ProgressService] Migration complete, old record deleted")

            except UserProgress.DoesNotExist:
                pass  # No old progress to migrate
            except UserProgress.MultipleObjectsReturned:
                logger.warning(
                    f"[ProgressService] Multiple old progress records found for user {submission.user.username}"
                )
    ...
    def _update_scores(progress, submission):
        ...
        if submission.score > progress.best_score:
            ...
            logger.info(
                f"[ProgressService] Updated best score to {progress.best_score}"
            )
    ...
    def _update_attempts(progress, submission):
        ...
        if submission.score == 100:
            progress.successful_attempts = (
                getattr(progress, "successful_attempts", 0) + 1
            )
            progress.consecutive_successes = (
                getattr(progress, "consecutive_successes", 0) + 1
            )
        else:
    ...
    def _create_daily_snapshot(progress, submission):
        ...
        if created:
            logger.info(
                f"[ProgressService] Created snapshot for {progress.user.username} on {progress.problem.slug}"
            )
        else:
            logger.info(
                f"[ProgressService] Updated snapshot for {progress.user.username} on {progress.problem.slug}"
            )
    ...
    def _emit_progress_event(progress, submission):
        ...
        """
        logger.info(
            f"[ProgressService] SSE Event - Progress updated for {progress.user.username} on {progress.problem.slug}"
        )
