...
class SegmentationService:
    ...
    def segment_prompt(
        ...
        try:
            ...
            content = self._call_ai(messages=messages, max_tokens=1500, temperature=0.0)
            ...
            level, feedback = self._determine_comprehension_level(
                segment_count, threshold, config
            )
...
```"""
        ...
        if custom_examples:
            ...
            if "relational" in custom_examples and self._is_valid_example(
                custom_examples["relational"]
            ):
                ...
                prompt_parts.append(
                    self._format_example_segments(
                        example["segments"], example["code_lines"]
                    )
                )

            # Add multi-structural example if it exists
            if "multi_structural" in custom_examples and self._is_valid_example(
                custom_examples["multi_structural"]
            ):
...
CRITICAL: Each line number must appear in at most ONE segment. Overlapping line numbers will invalidate the segmentation."""
    ...
    def _validate_segment_is_verbatim(
        self, segment_text: str, user_prompt: str
    ) -> bool:
        """
    ...
    def _parse_segments(
        self, ai_response: str, reference_code: str, user_prompt: str
    ) -> dict:
        ...
        try:
            ...
            if not segments:
                return {
                    "success": False,
                    "error": "No segments found in response",
                    "segments": [],
                    "groups": [],
                }
            ...
            for segment in segments:
                ...
                for line in lines:
                    ...
                    if line in used_lines:
                        logger.warning(
                            f"Line {line} already assigned to another segment - skipping to maintain one-to-one mapping"
                        )
                        continue
                ...
                if not valid_lines:
                    logger.warning(
                        f"Segment '{text[:50]}...' has no valid unique lines - skipping"
                    )
                    continue
                ...
                # EiPL Grader format
                valid_groups.append({"explanation_portion": text, "code": code_snippet})

            # Log one-to-one mapping statistics
            total_lines_mapped = len(used_lines)
            logger.info(
                f"Segmentation complete: {len(valid_segments)} segments mapping {total_lines_mapped} unique lines"
            )
    ...
    def _determine_comprehension_level(
        self, segment_count: int, threshold: int = 2, config: dict = None
    ) -> tuple:
        """
    ...
    def _log_segmentation_debug(
        self, user_prompt: str, reference_code: str, ai_response: str
    ) -> None:
