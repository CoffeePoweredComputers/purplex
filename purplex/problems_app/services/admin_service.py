...
class AdminProblemService:
    ...
    def create_problem_with_relations(
        ...
        for slug in problem_set_slugs:
            ...
            if problem_set:
                ...
                order = ProblemSetMembershipRepository.count_problem_set_memberships(
                    problem_set
                )
                ...
                logger.warning(
                    f"Problem set with slug '{slug}' not found during problem creation"
                )
    ...
    def prepare_problem_data(request_data: dict) -> Tuple[dict, List[str]]:
        ...
        """
        data = (
            request_data.copy() if hasattr(request_data, "copy") else dict(request_data)
        )
    ...
    def validate_problem_set_title(
        ...
        if current_slug and potential_slug != current_slug:
            ...
            not current_slug
            and ProblemRepository.get_problem_set_by_slug(potential_slug) is not None
        ):
            return f"A problem set with similar title already exists (slug: {potential_slug})"
    ...
    def create_problem_set_with_problems(
        ...
        if missing_problems:
            logger.warning(
                f"Problem set created but some problems not found: {missing_problems}"
            )
    ...
    def update_problem_set_with_problems(
        ...
        if missing_problems:
            logger.warning(
                f"Problem set updated but some problems not found: {missing_problems}"
            )
    ...
    @staticmethod
    def create_course_problem_set(
        course: "Course", problem_set: "ProblemSet", order: int = 0
    ) -> "CourseProblemSet":
        """
    ...
    def get_test_case(test_case_id: int, problem_slug: str) -> Optional["TestCase"]:
        ...
        return ProblemRepository.get_problem_test_case_by_id(
            problem, test_case_id, include_hidden=True
        )
