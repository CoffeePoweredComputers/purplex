...
class ResearchExportService:
    ...
    def _export_submissions(
        ...
        for submission in queryset:
            submission_data = {
                ...
                'problem_set_slug': submission.problem_set.slug if submission.problem_set else None,
                ...
                'time_spent_seconds': submission.time_spent.total_seconds() if submission.time_spent else None,

                # Test results
                'test_results': cls._format_test_results(submission.test_executions.all()),

                # Hint activations
                'hints_activated': cls._format_hint_activations(submission.hint_activations.all()),
            }
    ...
    def _export_progress(
        ...
        for progress in queryset:
            progress_data.append({
                ...
                'total_time_spent_seconds': progress.total_time_spent.total_seconds() if progress.total_time_spent else 0,
                'first_attempt': progress.first_attempt.isoformat() if progress.first_attempt else None,
    ...
    def _export_progress_history(
        ...
        for snapshot in queryset:
            snapshots.append({
                ...
                'problem_set_slug': snapshot.problem_set.slug if snapshot.problem_set else None,
                ...
                'time_spent_today_seconds': snapshot.time_spent_today.total_seconds() if snapshot.time_spent_today else 0,
            })
    ...
    def _export_hint_usage(
        ...
        for activation in queryset:
            hint_usage.append({
                'user_id': cls._anonymize_user_id(activation.submission.user, anonymize),
                'problem_slug': activation.submission.problem.slug,
                'problem_set_slug': activation.submission.problem_set.slug if activation.submission.problem_set else None,
