...
from purplex.problems_app.models import (
    Problem, ProblemSet, ProblemCategory, TestCase,
    ProblemSetMembership, Course, CourseProblemSet, CourseEnrollment,
...
class Command(BaseCommand):
    help = 'Populate the database with comprehensive sample data showcasing all features'
...
- `quick_sort([5, 2, 8, 1])` → `[1, 2, 5, 8]`''',
    ...
    return quick_sort(left) + middle + quick_sort(right)''',
                'test_cases': [
                    ([[3, 1, 4, 1, 5, 9]], [1, 1, 3, 4, 5, 9], 'Mixed values', False, True),
...
- `multiply([[1, 2], [3, 4]], [[5, 6], [7, 8]])` → `[[19, 22], [43, 50]]`''',
    ...
    return result''',
                'test_cases': [
                    ([[[1, 2], [3, 4]], [[5, 6], [7, 8]]], [[19, 22], [43, 50]], 'Basic multiplication', False, True),
                    ([[[1, 0], [0, 1]], [[5, 6], [7, 8]]], [[5, 6], [7, 8]], 'Identity matrix', False, True),
                ],
                'hints': []  # No hints for this problem
            }
        ]
        ...
        for data in problem_data:
            ...
            # Create test cases
            for i, (inputs, expected, desc, hidden, sample) in enumerate(data['test_cases']):
        ...
        sets_data = [
            {
                ...
                'problems': ['palindrome-checker', 'valid-parentheses', 'prime-checker']
            },
            {
                'slug': 'string-algorithms',
                'title': 'String Algorithms',
                'description': 'Master string manipulation techniques',
                'problems': ['palindrome-checker', 'anagram-detector', 'string-compression']
            },
            {
            ...
            },
            {
                ...
                'problems': ['binary-search', 'quick-sort', 'fibonacci-dp', 'gcd-calculator', 'matrix-multiplication']
            }
        ]
    ...
    def create_courses(self, instructors, students, problem_sets):
        ...
        for order, ps_slug in enumerate(['introduction-to-programming', 'string-algorithms']):
        ...
        for order, ps_slug in enumerate(['data-structures-fundamentals', 'algorithm-mastery']):
            ps = ProblemSet.objects.get(slug=ps_slug)
            CourseProblemSet.objects.get_or_create(
                course=cs201,
                problem_set=ps,
