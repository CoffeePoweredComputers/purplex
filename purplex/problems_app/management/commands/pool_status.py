...
class Command(BaseCommand):
    ...
    def show_status(self, json_output=False, compact=False):
        ...
        pool_containers = [
            c for c in all_containers if c.labels.get("purplex-pool") == "true"
        ]
        ...
        # Memory info (psutil already imported at top if available)
        if PSUTIL_AVAILABLE:
    ...
    def print_compact(self, data):
        ...
        mem_str = (
            f"{mem.get('free_gb', '?')}GB free" if "free_gb" in mem else "mem: N/A"
        )
    ...
    def print_formatted(self, data):
        ...
        orphan_style = (
            self.style.SUCCESS if containers["orphaned"] == 0 else self.style.WARNING
        )
        self.stdout.write(
            f"  Orphaned:              {orphan_style(str(containers['orphaned']))} containers {orphan_status}"
        )
        self.stdout.write(
            f"  Max allowed:           {containers['max_allowed']} containers\n"
        )
        ...
        if metrics:
            ...
            if pool_max > 0:
                ...
                self.stdout.write(
                    f"  Pool utilization:      {utilization:.1f}% ({pool_max - pool_size}/{pool_max} in use)"
                )
            ...
            if total_created > 0:
                ...
                self.stdout.write(
                    f"  Containers removed:    {metrics.get('total_removed', 0)}"
                )
            ...
            if health_checks > 0:
                ...
                self.stdout.write(
                    f"  Unhealthy removed:     {metrics.get('unhealthy_containers_removed', 0)}"
                )
        ...
        if "free_gb" in mem:
            ...
            mem_style = (
                self.style.SUCCESS if mem["percent"] < 75 else self.style.WARNING
            )
            mem_percent = f"{mem['percent']}%"
            self.stdout.write(
                f"  Usage:                 {mem_style(mem_percent)} {mem_status}\n"
            )
