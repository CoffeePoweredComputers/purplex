...
def prune_orphaned_containers():
    ...
    try:
        ...
        exited_containers = client.containers.list(
            all=True, filters={"status": "exited", "label": "purplex-pool=true"}
        )
        ...
        created_containers = client.containers.list(
            all=True, filters={"status": "created", "label": "purplex-pool=true"}
        )
        ...
        for container in created_containers:
            try:
                ...
                error_count += 1
                logger.warning(
                    f"Failed to remove created container {container.id[:12]}: {e}"
                )
        ...
        if removed_count > 0:
            logger.info(
                f"Cleanup task: removed {removed_count} orphaned containers, {error_count} errors"
            )
        else:
            logger.debug("Cleanup task: no orphaned containers found")
