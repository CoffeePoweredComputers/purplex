...
class CourseEnrollmentRepository(BaseRepository):
    ...
    @classmethod
    def get_active_enrollment(cls, user: User, course: Course) -> Optional[CourseEnrollment]:
    ...
    @classmethod
    def get_user_enrollments(cls, user: User, active_only: bool = True) -> List[CourseEnrollment]:
    ...
    @classmethod
    def get_course_enrollments(cls, course: Course, active_only: bool = True) -> List[CourseEnrollment]:
    ...
    @classmethod
    def bulk_enroll_users(cls, users: List[User], course: Course) -> List[CourseEnrollment]:
    ...
    @classmethod
    def get_recent_enrollments(cls, course: Course, days: int = 30) -> List[CourseEnrollment]:
    ...
    def get_enrollment_statistics(cls, course: Course) -> Dict[str, Any]:
        ...
        stats.update({
            ...
            'enrollment_rate': (stats['active_enrollments'] / stats['total_enrollments'] * 100) 
                              if stats['total_enrollments'] > 0 else 0
        })
    ...
    def get_instructor_enrollment_overview(cls, instructor: User) -> Dict[str, Any]:
        ...
        instructor_courses = Course.objects.filter(instructor=instructor, is_active=True)
    ...
    @classmethod
    def get_student_coursemates(cls, user: User, course: Course) -> List[CourseEnrollment]:
    ...
    def get_common_courses(cls, user1: User, user2: User) -> List[CourseEnrollment]:
        ...
        return list(CourseEnrollment.objects.filter(
            user=user2,
            is_active=True,
            course_id__in=user1_courses
        ).select_related('course'))
    
    @classmethod
    def search_enrolled_students(cls, course: Course, query: str) -> List[CourseEnrollment]:
    ...
    @classmethod
    def get_enrollment_timeline(cls, course: Course, days: int = 90) -> List[Dict[str, Any]]:
    ...
    def get_dropout_analysis(cls, course: Course) -> Dict[str, Any]:
        ...
        active_enrollments = CourseEnrollment.objects.filter(course=course, is_active=True).count()
        dropout_count = total_enrollments - active_enrollments
        
        dropout_rate = (dropout_count / total_enrollments * 100) if total_enrollments > 0 else 0
    ...
    @classmethod
    def get_capacity_info(cls, course: Course, max_capacity: Optional[int] = None) -> Dict[str, Any]:
