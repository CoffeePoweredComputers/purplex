...
from purplex.problems_app.models import Course, Problem, ProblemSet, UserProgress
...
class UserProgressRepository(BaseRepository):
    ...
    def get_or_create_progress(
        ...
        return UserProgress.objects.get_or_create(
            ...
            course=course,
            defaults={"problem_version": problem.version, "status": "not_started"},
        )
    ...
    def get_struggling_problems(
        ...
        return list(
            UserProgress.objects.filter(**filters).order_by("-attempts", "best_score")
        )
    ...
    def get_fastest_completions(
        ...
        return list(
            UserProgress.objects.filter(**filters).order_by("days_to_complete")[:limit]
        )
    ...
    def bulk_update_completion_status(
        ...
        with transaction.atomic():
            ...
            progress_records = UserProgress.objects.select_for_update().filter(
                **filters
            )
            updated_count = 0
    ...
    def get_learning_path_progress(
        ...
        progress_records = (
            UserProgress.objects.filter(user=user, course=course)
            .select_related("problem_set")
            .order_by("problem_set__order", "problem__title")
        )
    ...
    def reset_user_progress(
        ...
        try:
            with transaction.atomic():
                ...
                progress = cls.get_user_problem_progress(
    ...
    def get_user_aggregate_statistics(cls, user: User) -> Dict[str, Any]:
        ...
        Note: For detailed statistics with problem_set/course filtering,
        use get_user_statistics() instead.
