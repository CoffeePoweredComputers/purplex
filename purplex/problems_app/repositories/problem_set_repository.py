...
class ProblemSetRepository(BaseRepository):
    ...
    def get_all_problem_sets(cls) -> List:
        ...
        return list(ProblemSet.objects.all().select_related(
            'created_by'
        ).annotate(
    ...
    def get_problem_sets_by_difficulty(cls, difficulty: str) -> List:
        ...
        return list(ProblemSet.objects.filter(
            problems__difficulty=difficulty
    ...
    def get_problem_set_statistics(cls, problem_set: ProblemSet) -> Dict[str, Any]:
        ...
        difficulty_counts = problems.values('difficulty').annotate(
            count=Count('id')
        )
        ...
        category_counts = problems.values('categories__name').annotate(
    ...
    def get_required_problem_sets_in_course(cls, course: Course) -> List:
        ...
        return list(ProblemSet.objects.filter(
            courses=course,
            courseproblemset__is_required=True
        ).order_by('courseproblemset__order'))
    ...
    def get_optional_problem_sets_in_course(cls, course: Course) -> List:
        ...
        return list(ProblemSet.objects.filter(
            courses=course,
            courseproblemset__is_required=False
        ).order_by('courseproblemset__order'))
    
    @classmethod
    def clone_problem_set(cls, original_set: ProblemSet, user: User, 
                         new_title: str, new_slug: str) -> ProblemSet:
