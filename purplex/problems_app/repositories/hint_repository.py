...
class HintRepository(BaseRepository):
    ...
    def count_hints_for_problem(cls, problem: Problem) -> Dict[str, int]:
        ...
        """
        hints = ProblemHint.objects.filter(problem=problem).values('hint_type').distinct()
        result = {}
    ...
    def get_all_hint_types(cls) -> List[str]:
        ...
        """
        return list(ProblemHint.objects.values_list(
            'hint_type', flat=True
        ).distinct().order_by('hint_type'))
    
    @classmethod
    def create_hint(cls, problem: Problem, hint_type: str, content: str, 
                   **kwargs) -> ProblemHint:
        """
    ...
    @classmethod
    def delete_problem_hints(cls, problem: Problem, hint_type: Optional[str] = None) -> int:
        """
