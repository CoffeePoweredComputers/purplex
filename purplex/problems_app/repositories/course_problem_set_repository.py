...
class CourseProblemSetRepository(BaseRepository):
    ...
    @classmethod
    def get_by_course_and_problem_set(cls, course: Course, problem_set: ProblemSet) -> Optional[CourseProblemSet]:
        """Get a specific course-problem set relationship."""
        return CourseProblemSet.objects.filter(
            course=course,
            problem_set=problem_set
        ).select_related('course', 'problem_set').first()
    ...
    def get_course_problem_sets_ordered(cls, course: Course) -> List[CourseProblemSet]:
        ...
        return list(CourseProblemSet.objects.filter(
            course=course
        ).select_related('problem_set').order_by('order', 'added_at'))
    ...
    @classmethod
    def add_problem_set_to_course(cls, course: Course, problem_set: ProblemSet, 
                                 order: int = 0, is_required: bool = True,
                                 problem_set_version: int = 1) -> CourseProblemSet:
    ...
    @classmethod
    def remove_problem_set_from_course(cls, course: Course, problem_set: ProblemSet) -> bool:
        """
    ...
    @classmethod
    def update_problem_set_order(cls, course: Course, problem_set: ProblemSet, new_order: int) -> bool:
    ...
    @classmethod
    def update_problem_set_requirement(cls, course: Course, problem_set: ProblemSet, is_required: bool) -> bool:
    ...
    @classmethod
    def reorder_problem_sets(cls, course: Course, problem_set_orders: List[Dict[str, Any]]) -> bool:
        """
    ...
    @classmethod
    def get_course_problem_sets_with_stats(cls, course: Course) -> List[CourseProblemSet]:
        """Get course problem sets with related statistics."""
        return list(CourseProblemSet.objects.filter(
            course=course
        ).select_related('problem_set').prefetch_related(
            'problem_set__problems'
        ).order_by('order'))
    
    @classmethod
    def search_course_problem_sets(cls, course: Course, query: str) -> List[CourseProblemSet]:
        """Search problem sets within a course by title or description."""
        return list(CourseProblemSet.objects.filter(
            course=course
        ).select_related('problem_set').filter(
