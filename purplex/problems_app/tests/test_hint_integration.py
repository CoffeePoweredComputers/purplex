...
class HintWorkflowIntegrationTests(TransactionTestCase):
    ...
    def setUp(self):
        ...
        ProblemSetMembership.objects.create(
            problem_set=self.problem_set, problem=self.problem, order=1
        )
    ...
    def test_complete_hint_lifecycle_admin_to_student(self):
        ...
        hint_availability_url = reverse(
            "problem_hint_availability", kwargs={"slug": self.problem.slug}
        )
        ...
        hint_content_url = reverse(
            "problem_hint_detail",
            kwargs={"slug": self.problem.slug, "hint_type": "variable_fade"},
        )
        ...
        hint_content_url = reverse(
            "problem_hint_detail",
            kwargs={"slug": self.problem.slug, "hint_type": "subgoal_highlight"},
        )
        ...
        hint_content_url = reverse(
            "problem_hint_detail",
            kwargs={"slug": self.problem.slug, "hint_type": "suggested_trace"},
        )
    ...
    def test_hint_configuration_updates_with_existing_progress(self):
        ...
        response = self.client.get(
            hint_content_url, {"problem_set_slug": self.problem_set.slug}
        )
    ...
    def test_cross_course_hint_isolation(self):
        ...
        other_course = Course.objects.create(
            course_id="CS102", name="Advanced Programming", instructor=self.admin_user
        )

        CourseEnrollment.objects.create(
            user=self.student_user, course=other_course, is_active=True
        )
    ...
    def test_hint_system_error_handling(self):
        ...
        other_course = Course.objects.create(
            course_id="CS999", name="Not Enrolled", instructor=self.admin_user
        )
        ...
        hint_content_url = reverse(
            "problem_hint_detail",
            kwargs={"slug": self.problem.slug, "hint_type": "invalid_type"},
        )
    ...
    def test_hint_system_with_problem_set_without_course(self):
        ...
        response = self.client.get(
            hint_availability_url, {"problem_set_slug": self.problem_set.slug}
        )
    ...
    def test_hint_validation_edge_cases(self):
        ...
        hint = ProblemHint.objects.get(
            problem=self.problem, hint_type="subgoal_highlight"
        )
        self.assertEqual(hint.content["subgoals"][0]["title"], "🔧 初期化")
        self.assertEqual(
            hint.content["subgoals"][0]["explanation"], "Инициализация переменных"
        )
