...
class HintAPITests(TestCase):
    ...
    def setUp(self):
        ...
        ProblemSetMembership.objects.create(
            problem=self.problem, problem_set=self.problem_set, order=1
        )
        ...
        CourseEnrollment.objects.create(user=self.student_user, course=self.course)
    ...
    def test_admin_update_hints_success(self):
        ...
        variable_fade_hint = ProblemHint.objects.get(
            problem=self.problem, hint_type="variable_fade"
        )
    ...
    def test_hint_access_not_enrolled_course(self):
        ...
        other_course = Course.objects.create(
            course_id="OTHER101", name="Other Course", instructor=self.admin_user
        )
    ...
    @unittest.skip(
        "Concurrent test is unreliable - threading with test client has race conditions"
    )
        ...
        def update_hints():
            try:
                with transaction.atomic():
                    ...
                    url = reverse(
                        "admin_problem_hints", kwargs={"slug": self.problem.slug}
                    )
                    response = self.client.put(url, hints_data, format="json")
