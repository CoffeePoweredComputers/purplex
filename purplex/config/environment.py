...
class Config:
    ...
    def allowed_hosts(self) -> list[str]:
        ...
        if not hosts or hosts == ["*"]:
            raise ConfigurationError(
                "Production requires DJANGO_ALLOWED_HOSTS environment variable with specific IPs/domains (not wildcard '*'). Example: DJANGO_ALLOWED_HOSTS=54.123.45.67,mydomain.com,localhost,127.0.0.1"
            )
        return hosts
    ...
    def server_url(self) -> str:
        """Internal server URL"""
        default = (
            "http://localhost:8000" if self.is_development else "https://purplex.com"
        )
    ...
    def openai_api_key(self) -> str | None:
        """OpenAI API key (works with any OpenAI-compatible provider)"""
    ...
    def openai_base_url(self) -> str | None:
        """Optional base URL for OpenAI-compatible APIs (e.g., VT ARC, Llama compat)"""
    ...
    def validate_configuration(self) -> None:
        ...
        if self.is_production:
            ...
            try:
                ...
                if not hosts or hosts == ["*"]:
                    errors.append(
                        "Production requires DJANGO_ALLOWED_HOSTS (not ALLOWED_HOSTS) with specific IPs/domains"
                    )
            except ConfigurationError as e:
                errors.append(str(e))
                ...
                for sensitive in sensitive_vars:
                    if sensitive in key.upper():
                        ...
                        if any(weak in value.lower() for weak in weak_values):
                            raise ConfigurationError(f"Weak value detected for {key}")
