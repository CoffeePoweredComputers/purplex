...
CODE_EXECUTION = {
    ...
    "RATE_LIMIT_PER_MINUTE": config.get_int(
        "RATE_LIMIT_SUBMIT_PER_MINUTE", 20
    ),  # Increased from 10
    "RATE_LIMIT_PER_HOUR": config.get_int(
        "CODE_EXEC_RATE_LIMIT_PER_HOUR", 300
    ),  # Increased from 100
    # Docker configuration
    "DOCKER_IMAGE": os.environ.get(
        "DOCKER_SANDBOX_IMAGE", "purplex/python-sandbox:latest"
    ),
    ...
    "DOCKER_TMPFS_SIZE": os.environ.get(
        "DOCKER_TMPFS_SIZE", "10m"
    ),  # Temporary filesystem size
    # Container pooling configuration
    "POOL_ENABLED": config.get_bool(
        "DOCKER_POOL_ENABLED", True
    ),  # Enable container pooling for better performance
    "POOL_SIZE": config.docker_pool_size,  # Environment-specific: 3 (dev) / 15 (prod)
    # Container pool health monitoring
    "POOL_HEALTH_CHECK_INTERVAL": config.get_int(
        "DOCKER_POOL_HEALTH_CHECK_INTERVAL", 60
    ),  # Check every 60 seconds
    "POOL_CONTAINER_MAX_AGE": config.get_int(
        "DOCKER_POOL_CONTAINER_MAX_AGE", 3600
    ),  # Rotate containers after 1 hour
    "POOL_MAX_RESTART_ATTEMPTS": config.get_int(
        "DOCKER_POOL_MAX_RESTART_ATTEMPTS", 3
    ),  # Max restarts before removal
    ...
    "FORBIDDEN_BUILTINS": [
        ...
        "setattr",
        "delattr",
        "hasattr",  # Prevent attribute access bypasses
...
RATE_LIMITS = {
    ...
    "ai_generation": {
        "per_minute": config.get_int(
            "AI_RATE_LIMIT_PER_MINUTE", 10
        ),  # Increased from 5 for beta
        "per_hour": config.get_int(
            "AI_RATE_LIMIT_PER_HOUR", 50
        ),  # Increased from 30 for beta
        "per_day": config.get_int(
            "AI_RATE_LIMIT_PER_DAY", 150
        ),  # Increased from 100 for beta
    },
    "authentication": {
        "login_per_hour": config.rate_limit_auth_per_minute
        * 60,  # Convert from per-minute to per-hour
