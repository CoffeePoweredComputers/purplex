...
if config.env.value != "production":
    raise ValueError(
        f"Production settings loaded but PURPLEX_ENV is {config.env.value}"
    )
...
DATA_UPLOAD_MAX_MEMORY_SIZE = config.get_int(
    "DATA_UPLOAD_MAX_MEMORY_SIZE", config.file_upload_max_memory_size
)
    ...
    def sentry_before_send(event, hint):
        ...
        try:
            ...
            if "exc_info" in hint:
                ...
                if (
                    "TimeoutError" in exc_type.__name__
                    and "redis" in str(exc_value).lower()
                ):
