...
if os.environ.get('USE_CLOUDWATCH', 'false').lower() == 'true':
    ...
    LOGGING['loggers']['purplex']['handlers'].append('cloudwatch')
...
if os.environ.get('USE_ELASTICACHE', 'false').lower() == 'true':
    ...
    CACHES['default']['LOCATION'] = f'redis://{REDIS_HOST}:6379/1'
...
if os.environ.get('USE_SECRETS_MANAGER', 'false').lower() == 'true':
    ...
    if secrets:
        SECRET_KEY = secrets.get('django_secret_key', SECRET_KEY)
        DATABASES['default']['PASSWORD'] = secrets.get('db_password', DATABASES['default']['PASSWORD'])
        OPENAI_API_KEY = secrets.get('openai_api_key', OPENAI_API_KEY)
