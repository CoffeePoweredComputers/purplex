        ...
        <div v-else-if="enrollment.course.problem_sets && enrollment.course.problem_sets.length > 0" class="gallery-grid">
          ...
          >
                ...
                <div class="progress-bar-container">
                  <div class="progress-bar-background">
                    <div 
                      ...
                      :style="{ width: getProgressPercentage(psData.problem_set, enrollment.course.course_id) + '%' }"
                    ></div>
                  </div>
                  <span class="progress-text">
                    <template v-if="getTotalProblems(psData.problem_set) === 0">
                      No problems yet
                    </template>
                    <template v-else>
                      {{ getCompletedCount(psData.problem_set, enrollment.course.course_id) }} / 
                      {{ getTotalProblems(psData.problem_set) }} completed
                    </template>
                  </span>
                </div>
              </div>
            ...
            <div class="card-hover-text">
              {{ isCompleted(psData.problem_set, enrollment.course.course_id) ? 'Review →' : 
                 getCompletedCount(psData.problem_set, enrollment.course.course_id) > 0 ? 'Continue →' : 'Start →' }}
            </div>
          </div>
        </div>
...
export default {
  ...
  setup() {
    ...
    const getProgressPercentage = (problemSet, courseId = null) => {
      // TODO: Implement actual progress tracking for individual problem sets
      // For now, return 0 as a placeholder
      return 0
    }
    
    const getCompletedCount = (problemSet, courseId = null) => {
      // TODO: Implement actual completed count for individual problem sets  
      // For now, return 0 as a placeholder
      return 0
    }
    
    const getTotalProblems = (problemSet) => {
      return problemSet.problems_count || 0
    }
    
    const isCompleted = (problemSet, courseId = null) => {
      return getProgressPercentage(problemSet, courseId) === 100
    }
