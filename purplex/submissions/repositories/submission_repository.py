...
from ..models import Submission, HintActivation, CodeVariation, SubmissionFeedback
...
class SubmissionRepository(BaseRepository[Submission]):
    ...
    @classmethod
    def get_error_stats_for_problem(cls, course, problem, limit: int = 5) -> List[Dict[str, Any]]:
        """
        ...
        return list(TestExecution.objects.filter(
    ...
    def get_for_student_detail(cls, course, user, limit: int = 20):
        ...
        return Submission.objects.filter(
            course=course,
            user=user
        ).select_related('problem_set', 'problem').order_by('-submitted_at')[:limit]

    @classmethod
    def get_for_research_export(cls, course=None, problem_set=None,
    ...
    def get_latest_per_problem_in_set(cls, user, problem_set, course=None):
        ...
        return queryset.prefetch_related('segmentation').order_by(
    ...
    def get_user_submission_history(
        ...
        return list(Submission.objects.filter(
            **filters
            ...
            'problem_set', 'course', 'segmentation'
        ).prefetch_related(
            'code_variations',
            'test_executions'
