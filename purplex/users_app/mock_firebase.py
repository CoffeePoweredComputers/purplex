...
class MockFirebaseAuth:
    ...
    @classmethod
    def create_user(
        cls, email: str, password: str = None, display_name: str = None
    ) -> "MockUserRecord":
        """
    ...
    def get_user(cls, uid: str) -> "MockUserRecord":
        ...
        for email, data in cls.TEST_USERS.items():
            if data["uid"] == uid:
                return MockUserRecord(uid=uid, email=email, display_name=data["name"])

        # For development, allow any UID
        return MockUserRecord(uid=uid, email=f"{uid}@test.local", display_name=uid)
    ...
    def get_user_by_email(cls, email: str) -> "MockUserRecord":
        ...
        if email in cls.TEST_USERS:
            ...
            return MockUserRecord(
                uid=data["uid"], email=email, display_name=data["name"]
            )
    ...
    def list_users(
        ...
        for email, data in cls.TEST_USERS.items():
            users.append(
                MockUserRecord(uid=data["uid"], email=email, display_name=data["name"])
            )
