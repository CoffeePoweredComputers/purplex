...
class AuthenticationService:
            ...
            try:
                ...
                if not hasattr(settings, "_firebase_initialized"):
                    try:
                        ...
                        logger.info(
                            f"Firebase initialized with credentials from {firebase_path}"
                        )
                ...
                logger.error(
                    "firebase-admin not installed. Install it for production use."
                )
                raise
    ...
    def authenticate_token(cls, token: str) -> tuple[User, dict[str, Any]]:
            ...
            if cached_data:
                ...
                if token_exp > current_time:
                    ...
                    grace_period_seconds = getattr(
                        settings, "FIREBASE_GRACE_PERIOD_SECONDS", 600
                    )
                        ...
                        cached_data_with_expiry = {
                            ...
                            **expiry_info,
                            ...
                            "grace_period_remaining": int(
                                grace_period_seconds - time_since_expiry
                            ),
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"Cached token for user {firebase_uid} (TTL: {cache_ttl // 60} min)"
                )
            except Exception as e:
            ...
            if config.is_development:
                ...
                if expiry_info["needs_refresh"]:
                    logger.debug(
                        f"Token expiring soon: {expiry_info['expires_in']}s remaining"
                    )
        ...
        try:
            ...
            if cached_user_id:
                ...
                if user:
                    ...
                    logger.debug(
                        f"Cached user no longer exists, cleared cache for {firebase_uid}"
                    )
        except Exception as e:
    ...
    def get_or_create_user(
        ...
        @retry_with_backoff(
            max_retries=3, initial_delay=0.05, max_delay=2.0, backoff_factor=2.0
        )
        ...
        if duration > 0.1:  # Log operations taking more than 100ms
            logger.warning(
                f"Slow user creation/lookup: {duration:.3f}s for {firebase_uid}"
            )

        if user_profile.was_created:
            logger.info(
                f"Created new user: {user.username} (Firebase UID: {firebase_uid})"
            )
    ...
    def _update_user_if_needed(cls, user: User, email: str, display_name: str) -> None:
        ...
        if updated:
            user.save()
            logger.debug(
                f"Updated user {user.username}: email={email}, name={display_name}"
            )
    ...
    @classmethod
    def _apply_test_user_permissions(
        cls, user: User, user_profile: UserProfile, email: str
    ) -> None:
        """
        ...
        if email in MockFirebaseAuth.TEST_USERS:
            ...
            logger.debug(
                f"Applied test user permissions for {email}: "
    ...
    def verify_service_account(cls, service_key: str) -> User | None:
        ...
        if not hmac.compare_digest(service_key, valid_key):
            ...
            logger.warning("Failed service account authentication attempt")
            return None
        ...
        if not service_user:
            if config.is_development:
                ...
                UserProfileRepository.create(
                    user=service_user, firebase_uid="service-account", role="service"
                )
            else:
                return None
    ...
    def create_sse_session(cls, user: User) -> str:
        ...
        try:
            ...
            if verify:
                logger.info("✅ SSE session stored successfully in cache")
                logger.info(f"   Key: {session_key}")
            else:
                logger.error("❌ Failed to store SSE session in cache!")
        except Exception as e:
    ...
    def validate_sse_session(cls, session_token: str) -> User | None:
        ...
        logger.info(
            f"🔍 Validating SSE session token: {session_token[:10]}...{session_token[-10:]}"
        )
        ...
        try:
            ...
            logger.info("⏱️ Refreshed TTL for session (300 seconds)")

            # Get user from database using repository
            user = UserRepository.get_by_id(user_id)
            if not user:
                logger.error(
                    f"❌ User {user_id} NOT FOUND in database for valid SSE session"
                )
                cache.delete(session_key)  # Clean up orphaned session
                return None

            logger.info(
                f"✅ SSE session validated successfully for user: {user.username}"
            )
            return user
    ...
    def revoke_sse_session(cls, session_token: str) -> bool:
        ...
        try:
            ...
            cache.delete(session_key)
            if existed:
                logger.info("Revoked SSE session")
            return existed
        except Exception as e:
