...
class PerformanceTest:
    ...
    async def run_test(self):
        ...
        print(
            f"Starting performance test with {self.num_users} users for {self.duration} seconds"
        )
        ...
        async with aiohttp.ClientSession(
            ...
            self.results["requests_per_second"] = (
                self.results["success_count"] / total_time
            )
    ...
    def analyze_results(self):
        ...
        print(
            f"Success Rate: {(self.results['success_count'] / total_requests * 100):.2f}%"
        )
        ...
        if self.results["response_times"]:
            ...
            print(
                f"  95th Percentile: {sorted(response_times)[int(len(response_times)*0.95)]*1000:.2f} ms"
            )
            print(
                f"  99th Percentile: {sorted(response_times)[int(len(response_times)*0.99)]*1000:.2f} ms"
            )
        ...
        if self.results["error_count"] / total_requests > 0.01:  # More than 1% errors
            ...
            statistics.mean(self.results["response_times"]) > 1.0
...
async def main():
    ...
    parser.add_argument(
        "--url", default="http://localhost:8000", help="Base URL of Purplex"
    )
    parser.add_argument(
